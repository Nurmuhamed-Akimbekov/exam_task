...
create type MaritalStatus  as enum ('Single', 'Married', 'Divorced', 'Widow', 'Separated', 'Other');
...
alter table customers drop column family_status;
create table house
(
...
alter table house add  column id serial primary key ;
alter table house rename  to houses;

create table rent_info
(   owner_id    int references owners (id),
    address_id  int references addresses (id),
    agencies_id int references agencies(id),
    house_id int references  houses(id),
    customer_id int references customers(id),
    agency_id int references agencies(id));

alter table rent_info add column id serial primary key ;
alter table rent_info drop column agencies_id;
...
from houses h
         ...
         inner join agencies a2 on a2.id = ri.agency_id
...
select house_type, rating, concat(first_name, ' ', last_name) as owner_name
from houses
...
select count(*) as apartment_count, sum(houses.price) as all_sum_apartments
from houses
...
select a.name                                   as agienci_name,
       a2.street                                as agienci_address,
       concat(h.house_type, ' ', h.description) as house,
       h.address_id
from agencies a
         inner join rent_info ri on a.id = ri.agency_id
         inner join houses h on ri.house_id = h.id
         inner join addresses a2 on a2.id = ri.agency_id
where a.name = 'My_House';
select a.name, ad.*, h.id as house_address
from agencies a
         inner join addresses ad on ad.id = a.address_id
         inner join rent_info ri on a.id = ri.agency_id
         inner join houses h on ad.id = h.address_id
where a.name = 'Get House';
-- 9. Уйлордун арасынан мебели бар уйлорду, уйдун ээсин жана уйдун адрессин чыгар.
select concat(h.house_type, ' ', h.description) as house,
       concat(first_name, ' ', last_name)       as owner_name,
       a.street                                    as_house_address
from houses h
         inner join addresses a on a.id = h.address_id
         inner join owners o on o.id = h.owner_id
where furniture is true;
-- 10.Кленти жок уйлордун баарын жана анын адрессин жана ал уйлор кайсыл агентсвога тийешелуу экенин чыгар.
select concat(h.house_type, ' ', h.description) as house, a.street as address, a2.name as aagenci
from houses h
         inner join addresses a on a.id = h.address_id
         inner join rent_info ri on h.id = ri.house_id
         inner join agencies a2 on a2.id = ri.agency_id
where h.id not in (select
                       r.house_id from rent_info r
                                           inner join customers c on c.id = r.customer_id);
-- 11.Клиенттердин улутуна карап, улутуну жана ал улуутта канча клиент жашайт санын чыгар.
select c.nationality, count(*) as count_of_customer
from customers c
...
select *
from houses
where rating in (select max(houses.rating) from houses);
select *
from houses
where rating in (select min(rating) from houses);
-- 13.Уйлору жок киленттерди, клиенттери жок уйлорду чыгар.
select concat(h.house_type, ' ', h.description) as house, c.first_name as customer
from houses h
         full join rent_info ri on h.id = ri.house_id
         full join customers c on c.id = ri.customer_id;
-- 14.Уйлордун бааларынын орточо суммасын чыгар.
select avg(price) as avuge_summ_of_house_price
from houses;
...
select concat(o.first_name, ' ', o.last_name) as owner_name, count(h.id) as count_of_house
from owners o
         inner join rent_info ri on o.id = ri.owner_id
         inner join houses h on o.id = h.owner_id
group by owner_name
...
select h.house_type,h.description,ri.check_in,ri.check_out
from houses h
         inner join rent_info ri on h.id = ri.house_id
...
from houses h
         inner join owners o on o.id = h.owner_id
where h.price in (select max(price) from houses);
-- 23.Бир региондо жайгашкан баардык агентстволорду чыгарыныз
select region, count(*) as agience_count
from agencies a
...
select h.id        ,
       concat(h.house_type) as house,
       concat(a.city ,'  ',a.region ,' ',a.street) as house_address,
       avg(extract(year from age(o.date_of_birth)))AS average_owner_age
from houses h join owners o on h.owner_id = o.id
              join addresses a on h.address_id = a.id
group by  h.id, h.house_type, a.city, a.region, a.street limit 1;